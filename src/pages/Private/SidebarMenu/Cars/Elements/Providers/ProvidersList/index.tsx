...
import ProvidersCard from '../../../../../../../components/CarsCards/Elements/ProvidersCard';
...
const CarsProvidersList = () => {
      ...
      return (
        ...
        >
          {providersList.provider.map((elem, index) => (
            <ProvidersCard key={elem.id} provider={providersList.provider[index]}/>
          ))}
        </InfiniteScroll>);
    } else {
      return (
  ...
  return (
    <>
      ...
      <SelectionStyled justifyContent={'space-between'}>
        <Search name={ProvidersSearchHeaders} width={'962px'} />
        <Selection name={ProvidersSelection} values={cars} width={130} />
      </SelectionStyled>
      <ItemStyled>
        <ItemsHeadStyled gridTemplateColumns={'69px 367px 377px 413px 111px'}>
